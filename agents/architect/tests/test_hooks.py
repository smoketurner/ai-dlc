...
def test_write_spec_doc_blocked_before_read_memory_md() -> None:
    hooks = build_hooks()
    require = hooks[0]
    event = call(require, "write_spec_doc")
    assert event.cancel_tool is not None
    assert "read_memory_md" in event.cancel_tool


def test_write_spec_doc_allowed_after_read_memory_md() -> None:
    hooks = build_hooks()
    require = hooks[0]
