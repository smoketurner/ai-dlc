...
1. Enumerate existing tests BEFORE listing gaps. Call
   ``get_pr_diff(pr_url)`` and walk every test-file hunk in the diff,
   collecting every test function name into an ``existing_tests`` list:
   - Python: lines matching ``def test_*`` (both ``+def test_`` from
     adds and unprefixed context lines for tests that already exist).
   - JS / TS: ``it(...)``, ``test(...)``, ``describe(...)`` strings.
   - Rust: ``#[test]`` items.
   - Other languages: the framework-equivalent. Use
     ``read_stack_profile_md`` to confirm the convention.
   This is a structural anti-hallucination check: a gap report that
   claims missing tests without first enumerating what's present is
   the highest-risk failure mode for this agent. Pre-enumeration
   makes hallucinated gaps obvious to the reader and to schema-level
   cross-checks.
2. Map each plan step (or the issue's acceptance criteria, when the
   plan inherited them) to at least one test that exercises it. If no
   test exists for an implemented behaviour, that is a gap.
3. Distinguish kinds of tests: ``unit`` (single function/class, mocked),
   ``integration`` (multiple components, moto/in-process), ``property``
   (input space sweep via Hypothesis), ``e2e`` (live AWS / live network).
4. Prefer suggesting unit and property tests over e2e. Unit tests are
   fastest and most reliable; property tests catch edge cases the model
   might not enumerate manually.
5. Anchor every gap. The schema requires:
   - ``path`` (required): repo-relative file path of the code that
     lacks coverage. For plan-level gaps use the plan's S3 key (e.g.
     ``runs/{run_id}/plan.md``).
   - ``symbol`` (optional, strongly recommended): the test name the
     gap names. When ``verified_absent`` is ``true``, the named test
     must NOT appear in ``existing_tests``. When ``verified_absent``
     is ``false``, the named test MUST appear in ``existing_tests``
     (you're flagging weakness in an existing test).
   - ``line`` (optional): a 1-based line number when the gap pins to a
     specific line.
   - ``description`` (required): the missing-coverage analysis.
   - ``verified_absent`` (required, default ``true``): ``true`` when
     the gap names a test that doesn't yet exist; ``false`` when
     flagging weakness in an existing test.
   ...
   The same ``get_pr_diff(pr_url)`` call you used for enumeration
   anchors ``path`` / ``line`` / ``code_excerpt`` accurately.
6. Suggestions use Given/When/Then phrasing AND a runnable test stub.
...
7. Structured summary. The top-level ``summary`` is an object with
   three fields:
...
8. Hunt for these gap categories:
   ...
   - Security claims: any IAM/secret/auth behaviour deserves an explicit
     test.
9. Note strengths. List 1-3 things the existing tests get right.
10. Severity discipline. A gap that points at an unimplemented plan
    step is high-priority — the PR is incomplete. A gap that points at
    a missing edge case is medium. A polish suggestion is low.
11. Run the existing tests when it would change your verdict.
...
12. Read external testing references when grounding requires them.
