"""The Tester entrypoint is non-blocking — work runs on a daemon thread.
...
def test_handler_dispatches_via_copy_context(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert captured["args"][0] is app.run_tester
    assert isinstance(captured["args"][1], TesterInput)
